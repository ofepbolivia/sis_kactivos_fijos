...
require_once(dirname(__FILE__) . '/../reportes/RAFAsignadosFunPDF.php');
require_once(dirname(__FILE__) . '/../reportes/RAFAsignadosFunXls.php');
...
//        var_dump($this->res4);
        ...
        } else if ($this->objParam->getParametro('configuracion_reporte') == 'acti_fun_asignados') { //fRnk: HR00763
            $this->objFunc = $this->create('MODActivoFijo');
            $this->res_func = $this->objFunc->datosFuncionarios($this->objParam);
            $this->objFunc = null;
            $this->objFunc = $this->create('MODActivoFijo');
            $this->res_afa = $this->objFunc->reporteAFAsignadosFunc($this->objParam);
            if ($this->objParam->getParametro('formato_reporte') == 'pdf') {
                $nombreArchivo = uniqid(md5(session_id()) . '[Reporte - Activos Fijos Asignados]') . '.pdf';
            } else {
                $nombreArchivo = uniqid(md5(session_id()) . '[Reporte - Activos Fijos Asignados') . '.xls';
            }
        } else if ($this->objParam->getParametro('configuracion_reporte') == 'acti_fun_devueltos') {
            $this->objFunc = $this->create('MODActivoFijo');
            $this->res_func = $this->objFunc->datosFuncionarios($this->objParam);
            $this->objFunc = null;
            $this->objFunc = $this->create('MODActivoFijo');
            $this->res_afd = $this->objFunc->reporteAFDevueltosFunc($this->objParam);
            if ($this->objParam->getParametro('formato_reporte') == 'pdf') {
                $nombreArchivo = uniqid(md5(session_id()) . '[Reporte - Activos Fijos Devueltos]') . '.pdf';
            } else {
                $nombreArchivo = uniqid(md5(session_id()) . '[Reporte - Activos Fijos Devueltos') . '.xls';
            }
        }
        //Definicion de parametros adicionales para el reporte.
        $this->objParam->addParametro('tamano', 'LETTER');
        $this->objParam->addParametro('nombre_archivo', $nombreArchivo);
        $this->objParam->addParametro('titulo_archivo', '');
        ...
        if ($this->objParam->getParametro('configuracion_reporte') == 'compras_gestion') {
            ...
            if ($this->objParam->getParametro('formato_reporte') == 'pdf') {
                $this->objParam->addParametro('orientacion', 'L');
                $this->objReporteFormato = new RAFAsignadosFunPDF($this->objParam);
                $this->objReporteFormato->setDatos($this->res_afa->getDatos(), $this->objParam->getParametro('fecha_ini'), $this->objParam->getParametro('fecha_fin'), $this->res_func->getDatos(), $this->objParam->getParametro('configuracion_reporte'));
                $this->objReporteFormato->generarReporte();
                $this->objReporteFormato->output($this->objReporteFormato->url_archivo, 'F');
            } else {
                $reporte = new RAFAsignadosFunXls($this->objParam);
                $reporte->setDatos($this->res_afa->datos, $this->res_func->getDatos());
                $reporte->generarReporte();
            }
        } else if ($this->objParam->getParametro('configuracion_reporte') == 'acti_fun_devueltos') { //fRnk: HR00763
            if ($this->objParam->getParametro('formato_reporte') == 'pdf') {
                $this->objParam->addParametro('orientacion', 'L');
                $this->objReporteFormato = new RAFAsignadosFunPDF($this->objParam);
                $this->objReporteFormato->setDatos($this->res_afd->getDatos(), $this->objParam->getParametro('fecha_ini'), $this->objParam->getParametro('fecha_fin'), $this->res_func->getDatos(), $this->objParam->getParametro('configuracion_reporte'));
                $this->objReporteFormato->generarReporte();
                $this->objReporteFormato->output($this->objReporteFormato->url_archivo, 'F');
            } else {
                $reporte = new RAFAsignadosFunXls($this->objParam);
                $reporte->setDatos($this->res_afd->datos, $this->res_func->getDatos());
                $reporte->generarReporte();
            }
        }
